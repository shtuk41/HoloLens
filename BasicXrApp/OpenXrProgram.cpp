...
namespace {
    ...
    private:
        ...
        Hologram CreateHologram(const XrPosef& poseInAppSpace, XrTime placementTime) const {
            ...
            if (m_optionalExtensions.SpatialAnchorSupported) {
                ...
                if (XR_SUCCEEDED(result)) {
                    ...
                    CHECK_XRCMD(xrCreateSpatialAnchorSpaceMSFT(m_session.Get(), &createSpaceInfo, hologram.Sensor.Space.Put(xrDestroySpace)));
                ...
                CHECK_XRCMD(xrCreateReferenceSpace(m_session.Get(), &createInfo, hologram.Sensor.Space.Put(xrDestroySpace)));
            }
            return hologram;
        }
        ...
        void InitializeSensorHead(XrTime predictedDisplayTime) 
        {
            ...
            {
                // Initialize a sensor head 1 meter in front of user.
                Hologram hologram{};
                hologram.Sensor.Scale = {0.25f, 0.25f, 0.25f};
                hologram.Sensor.Space = createReferenceSpace(XR_REFERENCE_SPACE_TYPE_LOCAL, xr::math::Pose::Translation({0, 0, -1}));
                m_holograms.push_back(std::move(hologram));
                m_sensorHeadIndex = (uint32_t)m_holograms.size() - 1;

                m_spinningCubeStartTime = predictedDisplayTime;
            }
        }

        void UpdateSensorHead(XrTime predictedDisplayTime) 
        {
            if (!m_sensorHeadIndex) {
                // Deferred initialization of spinning cubes so they appear at right place for the first frame.
                InitializeSensorHead(predictedDisplayTime);
            }
            ...
            if (IsSessionFocused()) {
                ...
                //const float radius = 0.001f;                        // Rotation radius in meters

                // Let spinning cube rotate around the main cube's y axis.
                XrPosef pose;
                pose.position = {0, 0, 0};
                //{radius * std::sin(angle), 0, radius * std::cos(angle)};
                pose.orientation = xr::math::Quaternion::RotationAxisAngle({0, 1, 0}, angle);
                m_holograms[m_sensorHeadIndex.value()].Sensor.PoseInSpace = pose;
            }
        }
        ...
        bool RenderLayer(XrTime predictedDisplayTime, XrCompositionLayerProjection& layer) {
            ...
            std::vector<const sample::SensorHead*> visibleSensors;

            auto UpdateVisibleSensor = [&](sample::SensorHead& sensor) {
                if (sensor.Space.Get() != XR_NULL_HANDLE) {
                    XrSpaceLocation sensorSpaceInAppSpace{XR_TYPE_SPACE_LOCATION};
                    CHECK_XRCMD(xrLocateSpace(sensor.Space.Get(), m_appSpace.Get(), predictedDisplayTime, &sensorSpaceInAppSpace));

                    // Update cube's location with latest space location
                    if (xr::math::Pose::IsPoseValid(sensorSpaceInAppSpace)) {
                        if (sensor.PoseInSpace.has_value()) {
                            sensor.PoseInAppSpace = xr::math::Pose::Multiply(sensor.PoseInSpace.value(), sensorSpaceInAppSpace.pose);
                        } else {
                            sensor.PoseInAppSpace = sensorSpaceInAppSpace.pose;
                        }
            ...
            UpdateSensorHead(predictedDisplayTime);
            ...
            for (auto& hologram : m_holograms) {
                ...
                UpdateVisibleSensor(hologram.Sensor);
            }
        ...
        void PrepareSessionRestart() {
            m_mainCubeIndex = m_sensorHeadIndex = {};
            m_holograms.clear();
    ...
    private:
        ...
        std::optional<uint32_t> m_sensorHeadIndex;
